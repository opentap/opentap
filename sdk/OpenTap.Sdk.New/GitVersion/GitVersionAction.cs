...
namespace OpenTap.Sdk.New.GitVersion
{
    ...
    [OverrideCliAction(typeof(OpenTap.Package.GitVersionAction))]
    public class GitVersionAction : OpenTap.Cli.ICliAction
    {
        ...
        public int Execute(CancellationToken cancellationToken)
        {
            ...
            if (!String.IsNullOrEmpty(ReplaceFile))
            {
                ...
                {
                    log.Error("File '{0}' given in --replace argument could not be found.",
                        Path.GetFullPath(ReplaceFile));
                    return (int)ExitCodes.ArgumentError;
                }
                ...
                if (replaceLineCount == 0)
                    ...
                    log.Info("Replaced '$(GitVersion)' with '{0}' in {1} line(s) of {2}", versionString,
                        replaceLineCount, ReplaceFile);
                ...
                foreach (Commit c in History)
                {
                    ...
                    if (c.Parents.Count() > 1)
                    {
                        ...
                        if (commitPosition.ContainsKey(p1))
                            if (commitPosition[p1] != commitPosition[c])
                            {
                                ...
                                foreach (var kvp in commitPosition
                                             .Where((KeyValuePair<Commit, int> kvp) => kvp.Value == endPos).ToList())
                                {
                    ...
                    else
                    {
                        ...
                        if (commitPosition[p1] != commitPosition[c])
                        {
                            ...
                            foreach (var kvp in commitPosition
                                         .Where((KeyValuePair<Commit, int> kvp) => kvp.Value == startPos).ToList())
                            {
                                if (kvp.Key != p1)
                                    commitPosition.Remove(kvp.Key);
                            }
                        ...
                        if (c.Parents.Any())
                        {
                            ...
                            if (c.Parents.Count() > 1)
                            {
                                ...
                                if (commitPosition.ContainsKey(p1))
                                    if (commitPosition[p1] != commitPosition[c])
                                    {
                                        ...
                                        Console.WriteLine();
                                        commitPosition[c] = startPos;
                                        foreach (var kvp in commitPosition.Where((KeyValuePair<Commit, int> kvp) =>
                                                     kvp.Value == endPos).ToList())
                                        {
                                            commitPosition.Remove(kvp.Key);
                                        }
                                    }
                            ...
                            else
                            {
                                ...
                                if (commitPosition[p1] != commitPosition[c])
                                {
                                    ...
                                    commitPosition[p1] = startPos;
                                    foreach (var kvp in commitPosition
                                                 .Where((KeyValuePair<Commit, int> kvp) => kvp.Value == startPos)
                                                 .ToList())
                                    {
                                        if (kvp.Key != p1)
                                            commitPosition.Remove(kvp.Key);
