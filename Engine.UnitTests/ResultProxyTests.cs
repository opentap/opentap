    ...
    public class ResultProxyTests 
    {
        ...
        public class AdvancedResultsStep : TestStep
        {
            ...
            public class AdvancedResult
            {
                ...
                public static AdvancedResult New()
                {
                    return new AdvancedResult() {X = 3, Y = 5};
                }
            }
            public override void Run()
            {
                ...
                Results.Publish(new AdvancedResult[]{AdvancedResult.New(), AdvancedResult.New()});
            }
        }
        ...
        public void ResultTableAndColumnParameters()
        {
            ...
            {
                // this result was generated by manually building a ResultTable.
                var table = rl.Results[0];
                
                var x = table.Columns[0];
                var y = table.Columns[1];
                Assert.IsTrue(x.Parameters["Unit"].ToString() == "s");
                Assert.IsTrue(y.Parameters["Unit"].ToString() == "W");
                var comment = (string) table.Parameters["Comment"];
                Assert.IsTrue(comment.Length > 0);
            }
            {
                // this result was generated with reflection
                var result = rl.Results[1];
                Assert.IsTrue(((string) result.Parameters["Comment"]) == "ResultCommentTest");
                var c1 = result.Columns[0];
                Assert.AreEqual("X", c1.Name);
                Assert.AreEqual("s", c1.Parameters["Unit"]);
                var c2 = result.Columns[1];
                Assert.AreEqual("Y", c2.Name);
                Assert.AreEqual("W", c2.Parameters["Unit"]);
                Assert.AreEqual("ResultCommentTest2", c2.Parameters["Comment"]);
            }
        }
    }
}
