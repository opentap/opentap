...
namespace OpenTap
{
    ...
    public class ResultSource : IResultSource
    {
        ...
        class PublishResultTableInvokable : IInvokable<IResultListener>, IWorkQueueIntrospectiveIInvokable
        {
            ...
            ResultTable CreateOptimizedTable()
            {
                var workQueue = WorkQueue.CurrentWorkQueue.Value;
                List<ResultTable> mergeTables = null;
                while (workQueue?.Peek() is IWrappedInvokable wrap && wrap.InnerInvokable is PublishResultTableInvokable p)
                {
            ...
            public void Invoke(IResultListener a)
            {
                try
                {
                    a.OnResultPublished(proxy.stepRun.Id, CreateOptimizedTable());
                }
                catch (Exception e)
                {
                    log.Warning("Caught exception in result handling task.");
