...
namespace OpenTap
{
    ...
    static internal class Sequence
    {
        ...
        public readonly struct Either<T, T2>
        {
            readonly object value;

            public bool Unfold(out T trueValue, out T2 falseValue)
            {
                if (value is T vv1)
                {
                    trueValue = vv1;
                ...
                throw new Exception("Cannot unfold value.");
            }
            
            public void Match(Action<T> a, Action<T2> b)
            {
                if (value is T av) a(av);
                else if (value is T2 bv) b(bv);
                else throw new Exception("Cannot match value.");
            }

            public T3 MatchInto<T3>(Func<T, T3> a, Func<T2, T3> b)
            {
                if (value is T av) return a(av);
                if (value is T2 bv) return b(bv);
                throw new Exception("Cannot match value.");
            } 
            public Either(T v1) => value = v1;
            public Either(T2 v1) => value = v1;
        }
        ...
        public class SelectException : Exception
        {
            ...
            public SelectException(object Object, Exception inner) : base("SelectException: " + inner.Message, inner)
            {
                this.Object = Object;
            }
        }
        ...
        /// <summary>
        /// Select, but with grabbing exceptions for future use.
        /// </summary>
        public static IEnumerable<Either<T2, SelectException>> TrySelect<T, T2>(this IEnumerable<T> src, Func<T, T2> f)
        {
            foreach (var x in src)
            {
                Either<T2,SelectException> y;
                try
                {
                    y = new Either<T2, SelectException>(f(x));
                }
                catch(Exception e)
                {
                    y = new Either<T2, SelectException>(new SelectException(x, e));
                }
                yield return y;
            }
        }
        /// <summary> Handle exceptions generated by TrySelect. </summary>
        public static IEnumerable<T> Unwrap<T>(this IEnumerable<Either<T, SelectException>> src, Action<Exception> handler)
        {
            return src.Unwrap<T, Exception>((x,ex) => handler(ex));
        }
        
        /// <summary> Handle exceptions generated by TrySelect. </summary>
        public static IEnumerable<T> Unwrap<T,Tex>(this IEnumerable<Either<T, SelectException>> src, Action<Tex> handler) where Tex: Exception
        {
            return src.Unwrap((T x,Tex ex) => handler(ex));
        }
        
        /// <summary> Handle exceptions generated by TrySelect. </summary>
        public static IEnumerable<T> Unwrap<T>(this IEnumerable<Either<T, SelectException>> src, Action<T, Exception> handler)
        {
            return src.Unwrap<T, Exception>(handler);
        }

        /// <summary> Handle exceptions generated by TrySelect. </summary>
        public static IEnumerable<T> Unwrap<T,T2>(this IEnumerable<Either<T, SelectException>> src, Action<T, T2> handler) where T2: Exception
        {
            foreach (var x in src)
            {
                if (x.Unfold(out T value, out SelectException ex))
                {
                    yield return value;
                }
                else
                {
                    if (ex.InnerException is T2 ex2 && ex.Object is T obj)
                        handler(obj, ex2);
                    else throw new Exception("Unexpected exception unwrapped");
                }
            }
        }
    }
